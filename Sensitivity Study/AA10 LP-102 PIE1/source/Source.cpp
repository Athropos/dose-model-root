...
Source::Source():pName(NULL), 
                pHalfLife(1*TMath::Power(10,9)), 
                function(constant_release),
                release_duration(1.4*TMath::Power(10,6))
                {pNuclei = (int)pHalfLife/TMath::Log(2);}

Source::Source(string name, double HalfLife, double(*f)(double)): pName(name), 
                                                                pHalfLife(HalfLife), 
                                                                function(f),
                                                                release_duration(1.4*TMath::Power(10,6))
                                                                { pNuclei = (int)pHalfLife/TMath::Log(2);}

Source::Source(string name, double HalfLife): pName(name), 
                                            pHalfLife(HalfLife), 
                                            function(constant_release),
                                            release_duration(1.4*TMath::Power(10,6))
                                            {pNuclei = (int)pHalfLife/TMath::Log(2);}
...
double Source::ConstEval(double t){
    double tmin = 0;
    double tmax = release_duration;; //reference [1], 700 kg of water pumped at the rate of 1.80 Kg/hours 
    if(t<tmax){return 1/(tmax - tmin);} //return the activity at time x
    else {return 0;}
}
...
void Source::setRelease_function(double(*f)(double)){function = f;}
void Source::setRelease_duration( double t){release_duration = t;}
...
double Source::getRelease_duration() const {return release_duration;}