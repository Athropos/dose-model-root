...
void StandaloneApplication(int argc, char** argv) {
  ...
  // double a = 500;  //min
  // double b = 1640; //max equal to MAR, see [1]
  // double c = 700; //mode
  // TF1 waterPDF("waterPDF", WaterPDF, a, b, 3);
  // waterPDF.SetParameters(a, b, c);  

  //wind distribution from empirical data
  // TUnuran wind_random;
  // wind_random.Init(windEmpDist()); // See definition of windEmpDist in Utility

  
  TFile windDist("windDist.root");
  TH1F * wind_PDF = (TH1F*)windDist.Get("hWind");

  //deposition velocity distribution for C-14 piecewise uniform 
  TF1 deposition_vel_C14_PDF("deposition_vel_C14_PDF", Deposition_vel_C14_PDF, 0.001, 0.013, 0);

  //inhalation and ingestion coefficient for H-3 retrieved from bilognormal distribution
  TFile copula("copula.root");
  TH2F * hBilognormal = (TH2F*)copula.Get("hBilognormal");

  //inhalation rate (m3/day) and food consuption lognormal distribution from [4]
  TF1 *inh_rate_PDF = new TF1("inh_rate_PDF","ROOT::Math::lognormal_pdf(x,[0],[1])",0.1,33.86);
  ...
  for(int i=0; i< nAccidents; i++){
    ...
    double water_spilled = gRandom->Uniform(525, 875);//waterPDF.GetRandom();

    OuterSource outer_source;
    double activity_conc_H3 = gRandom->Gaus(2.94e+11, 0.05*2.94e+11);
    double activity_conc_C14 = gRandom->Gaus(5.93e+8, 0.05*5.93e+8);
    ...
    TF1 sigma_zPDF = Sigma_zPDF(stability_class);//See definition of Sigma_zPDF in Utility
    double sigma_z = sigma_zPDF.GetRandom();
    dispersion.setSigma_z(sigma_z);
    
    double wind_speed = wind_PDF->GetRandom();
    ...
    //vector<double> deposition_vel{0,0};
    vector<double> deposition_vel{gRandom->Uniform(0,0.007), deposition_vel_C14_PDF.GetRandom() };
    dispersion.setDeposition_vel(deposition_vel);

    dispersion.Go();

    Dose dose(dispersion);
    double lumped_par = gRandom->Uniform(0.0067, 0.01);
    dose.setLumped_translocation(lumped_par);
    double inh_coeff, ing_coeff, inh_rate, final_dose;
    hBilognormal->GetRandom2(inh_coeff, ing_coeff);
  ...
  TH1F * hDose = new TH1F("hDose", "Dose", 2500, 0, max_dose );
  TH1F * hDoseInhal = new TH1F("hDoseInhal", "Dose", 2500, 0, max_dose );
  TH1F * hDoseIng = new TH1F("hDoseIng", "Dose", 2500, 0, max_dose );
  TH1F * hDoseCS = new TH1F("hDoseCS", "Dose", 2500, 0, max_dose );
  TH1F * hDoseGS = new TH1F("hDoseGS", "Dose", 2500, 0, max_dose );
