...
//Function of the vertical profile for the dry deposition depletion factor, see [3]
...
Dispersion::Dispersion(OuterSource outer_source): STouter(outer_source),
    ...
    cout << "Insert wind speed (m/s): "; cin >> wind_speed ;
    //wind_speed = 3; //m/s
    cout << "Insert stability class (1-5): "; cin >> stability_class; 
    cout << "Insert release height (m): "; cin >> stack_height; //m
    cout << "Insert rain intensity (mm/h): "; cin >> rain_intensity; //mm/h
...
void Dispersion::Go(){
    ...
    ifstream in2;
    in2.open("Dose_Coeff");
    double cs_c, gs_c, inhal_c, ing_c, dep_vel,_A,_B;
    while(in2 >> name >> half_life >> cs_c >> gs_c >> inhal_c >> ing_c >> dep_vel >> _A >> _B){
    ...
    cout << "\nnuclide\t rel conc \trel g conc\tactivity \tdep vel\twash out\n";
    ...
    for(int i=0; i<Q.size(); i++){
        ...
        double wash_out_vel = wash_out.at(i)/(wind_speed*sigma_y*TMath::Sqrt(2*TMath::Pi()));
        double rel_ground_conc = rel_conc*(deposition_vel.at(i) + wash_out_vel );
        RGC.push_back(rel_ground_conc);
        cout << names.at(i) << "\t" << rel_conc << "\t" << rel_ground_conc << "\t" << Q.at(i) << "\t"<< deposition_vel.at(i) << "\t" << wash_out_vel << "\n";
    }
}
