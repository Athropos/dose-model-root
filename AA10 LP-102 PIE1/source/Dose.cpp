...
void Dose::Go(){
    ...
    for(int i=0; i<names.size(); i++){
        ...
        double _dose_gs = realeased_activity.at(i)*relative_ground_conc.at(i)*groundshine_coeff.at(i);
    ...
    for(int i=0; i<names.size(); i++){
        ...
        double lambda = TMath::Log(2)/half_lives.at(i);
        double _dose_ing = realeased_activity.at(i)*relative_ground_conc.at(i)*ing_coeff.at(i)*lumped_translocation*food_consuption;
